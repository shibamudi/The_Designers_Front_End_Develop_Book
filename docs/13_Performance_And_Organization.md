...
值 Values
现在我们已经通过一个选择器选择了一个元素，并通过属性来确定我们将应用什么样式。 现在我们可以通过值来确定属性的行为了。 值可以通过文本表示，放在冒号和分号之间，以下是我们选择了所有的<p> 元素，并设置了color属性，值为orange， font-size属性，值为16pixels.
p {
  color: orange;
  font-size: 16px;
}
...
图 1.03
CSS语法CSS syntax outline including a selector, properties, and values
